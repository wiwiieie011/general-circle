...
func (m *mockEventScheduleRepo) Create(s *models.EventSchedule) error                 { if m.CreateFunc != nil { return m.CreateFunc(s) }; return nil }
func (m *mockEventScheduleRepo) GetByID(id uint) (*models.EventSchedule, error)      { if m.GetByIDFunc != nil { return m.GetByIDFunc(id) }; return nil, nil }
func (m *mockEventScheduleRepo) GetByEventID(eid uint) ([]models.EventSchedule, error) { if m.GetByEventIDFunc != nil { return m.GetByEventIDFunc(eid) }; return nil, nil }
...
func TestSchedule_GetByEventID_Success(t *testing.T) {
    ...
    if err != nil || len(got) != 2 { t.Fatalf("unexpected: len=%d err=%v", len(got), err) }
}
...
func TestSchedule_Create_Success(t *testing.T) {
    now := time.Now()
    repo := &mockEventScheduleRepo{CreateFunc: func(s *models.EventSchedule) error { s.ID = 10; return nil }}
    evtRepo := &mockEventRepo{GetByIDFunc: func(id uint) (*models.Event, error) { return &models.Event{Base: models.Base{ID: id}}, nil }}
    svc := NewEventScheduleService(repo, evtRepo, logger())
    ...
    if got == nil || got.ID != 10 || got.EventID != 2 || got.ActivityName != "Talk" || got.Speaker != "Alice" { t.Fatalf("unexpected schedule: %#v", got) }
}
...
func TestSchedule_Create_BadTime(t *testing.T) {
    ...
    svc := NewEventScheduleService(repo, evtRepo, logger())
    _, err := svc.CreateScheduleForEvent(2, dto.CreateScheduleRequest{ActivityName: "Talk", Speaker: "Alice", StartAt: now, EndAt: now.Add(-time.Hour)})
    if err == nil || !errors.Is(err, e.ErrNotCorrectScheduleTime) { t.Fatalf("expected ErrNotCorrectScheduleTime, got %v", err) }
}
